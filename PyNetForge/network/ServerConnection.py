...
from PyNetForge.network.messages.ConnectedMessage import ConnectedMessage
from PyNetForge.network.messages.ConnectionProcessCrashedMessage import \
    ConnectionProcessCrashedMessage
from PyNetForge.network.messages.NetworkMessage import NetworkMessage
from PyNetForge.network.parser.MessageParser import MessageParser
...
class ServerConnection(mp.Thread):
    ...
    def onClose(self, err) -> None:
        ...
        from PyNetForge.network.messages.ServerConnectionClosedMessage import \
    ...
    def onConnectionTimeout(self) -> None:
        from PyNetForge.network.messages.ServerConnectionFailedMessage import \
            ...
            try:
                ...
                if rdata:
                    if self._connecting.is_set():
                        ...
                        self.onConnect()
                    self.stream += rdata
                    MessageParser().parse(self.stream, self.handleMessage)
                else:
